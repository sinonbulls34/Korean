...
function setupQuizModes() {
  document.querySelectorAll("#quizModeTabs [data-practice-view]").forEach((button) => {
    button.addEventListener("click", () => {
      const view = button.dataset.practiceView;
      document.querySelectorAll("#quizModeTabs [data-practice-view]").forEach((node) => {
      ...
      if (button.dataset.mode) {
        activeQuizMode = button.dataset.mode;
        newQuiz();
      }
    });
  });
...
function setupTabs() {
  ...
  } else if (document.querySelector(`#${hash}.learning-view`)) {
    activateTab("learning", false);
    activateLearningView(hash, false);
  } else if (document.querySelector(`#${hash}.practice-view`)) {
    activateTab("practice", false);
    document.querySelector(`[data-practice-view="${hash}"]`)?.click();
  }
}
