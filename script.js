...
function mouthSvg(type) {
  ...
  return `
    <svg class="mouth-svg" viewBox="0 0 78 64" aria-hidden="true" focusable="false">
...
function newQuiz() {
  ...
  const options = shuffle([
    answer,
    ...shuffle(quizPool.filter((item) => item.id !== answer.id)).slice(0, 3)
  ]);
  currentQuiz = answer;
  document.querySelector("#quizResult").textContent = "";
  document.querySelector("#quizPrompt").textContent = "聽音後選出正確的韓文字";
  document.querySelector("#quizHint").textContent = `${answer.roman}｜${answer.zh}`;
  document.querySelector("#answerGrid").innerHTML = options
    .map((item) => `<button type="button" data-answer="${item.id}">${item.symbol}<small>${item.sound}</small></button>`)
    .join("");
...
function checkAnswer(event) {
  ...
  document.querySelector("#quizResult").textContent = isCorrect
    ? "答對了。接著跟讀一次，把嘴型也做出來。"
    : `這題是 ${currentQuiz.symbol}，先慢速再聽一次。`;
  saveProgress();
}
...
function setupControls() {
  ...
  document.querySelector("#playQuiz").addEventListener("click", () => speak(currentQuiz.sound));
